...
- `scripts/verify_m1_acceptance.py` 已提供 M1 本地验收聚合入口，默认运行 brokerless 三端链路并汇总完整合并帧、ghost_probe 事件、`brake_decel > 0` 制动决策、路侧首次遮挡感知到云端 ghost_probe 事件的 `Lead-Time`（目标 1.5-3.0s）、fallback 恢复和 100ms 延迟目标；任一门槛失败会返回非零退出码，本地 `verify_all.ps1` 与 GitHub Actions 基础 CI 已纳入该检查。
...
- `RoadsideAgent`、`VehicleAgent`、`CloudAgent` 代码存在，Windows MQTT 三端联动脚本已补齐；无 Broker 时可用内存总线验证 topic 流、同帧合并、高危事件生成、制动决策、路侧提前预警 Lead-Time、端到端延迟目标和车端降级恢复状态；当前机器已用嵌入式 `amqtt` 完成真实 TCP Broker 闭环验证；GitHub Actions 已加入 Ubuntu Mosquitto 外部 Broker 验收；当前 Windows 本机有 Mosquitto/Docker 时仍可运行 `scripts/verify_mqtt_broker_demo.py` 完成本机外部 Broker 实机通过。
...
1. **MQTT 三端联动**
   ...
   - Roadside Agent + Vehicle Agent + Cloud Agent 完整链路（启动脚本已补齐；内存总线 topic/事件/制动/路侧提前预警 Lead-Time/fallback/延迟目标验证已完成；嵌入式 `amqtt` 真实 TCP Broker 验证已通过；外部 Mosquitto/Docker Broker 待具备环境后运行通过）
